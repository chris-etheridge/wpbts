...
SELECT TBL_EVENT.`EVENT_ID`, DATE_FORMAT(TBL_EVENT.`EVENT_DATE`, '%d-%m-%Y') AS EVENT_DATE, TBL_EVENT.`ADDRESS_ID`, TBL_EVENT.`TYPE_ID`, TBL_EVENT.`DESCRIPTION`, TBL_EVENT.`TITLE`, TBL_EVENT.`ACTIVE`, TBL_EVENT.`CREATOR_ID`, STREET_NO, CITY, OFFICE, STREET, AREA, AREA_CODE, BUILDING_NUMBER, TBL_EVENT_TYPE.DESCRIPTION AS TYPE_DESCRIPTION, URGENCY
FROM TBL_EVENT
